...
function search() { find . -type f -print | xargs grep "$1" }

# Docker containers
alias dup="ddown; (cd ~/dev-environments/air-local && docker-compose up -d)"
...
function log() { ( tail -f ~/dev-environments/air-local/mounts/log/eagleeye/$1/app/error.log) }
